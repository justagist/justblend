...
PYBIND11_MODULE(_core, m) {
    ...
    py::enum_<SegmentType>(m, "SegmentType")
        .value("LINEAR", SegmentType::Linear)
        .value("BLEND", SegmentType::Blend);

    py::class_<Limits>(m, "Limits")
        .def(py::init([](const Eigen::VectorXd& v_max, const Eigen::VectorXd& a_max,
        ...
        .def("__repr__", [](const Limits& l) {
            std::string s = "Limits(v_max=..., a_max=...";
            if (l.j_max.has_value()) s += ", j_max=...";
            s += ")";
            return s;
        });
    ...
    py::class_<Sample>(m, "Sample")
        ...
        .def("__repr__",
             [](const Sample& s) { return "Sample(t=" + std::to_string(s.t) + ", dim=" + std::to_string(s.q.size()) + ")"; });
    ...
    py::class_<Trajectory>(m, "Trajectory")
        ...
        .def(
            "samples",
            [](const Trajectory& self, const Eigen::VectorXd& times) { return self.samples(times); },
            py::arg("times"))
