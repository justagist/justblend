...
ScurveSample sampleScurve(double t_local, double v0, const std::vector<ScurvePhase>& phases)
{
    ...
    for (const auto& p : phases)
    {
        ...
        s += sd * p.duration + 0.5 * sdd * p.duration * p.duration +
             (1.0 / 6.0) * p.jerk * p.duration * p.duration * p.duration;
...
{
    if (rampDistance(v0, v_upper, amax, jmax) <= D)
        return v_upper;
    double lo = v0;
    double hi = v_upper;
    for (int i = 0; i < 80; ++i)
