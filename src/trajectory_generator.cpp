...
{
    if (dim_ == 0)
        throw ValidationError("TrajectoryGenerator: dim must be >= 1.");
}
...
void TrajectoryGenerator::setLimits(const Limits& limits)
{
    ...
    {
        throw ValidationError("setLimits: v_max size mismatches dim.");
    }
    if (static_cast<std::size_t>(limits.a_max.size()) != dim_)
    {
        throw ValidationError("setLimits: a_max size mismatches dim.");
    }
    if (limits.j_max.has_value() && static_cast<std::size_t>(limits.j_max->size()) != dim_)
    {
        throw ValidationError("setLimits: j_max size mismatches dim.");
    }
...
void TrajectoryGenerator::setOptions(const GenerationOptions& opts)
{
    ...
    {
        throw ValidationError("setOptions: blend_radius must be > 0.");
    }
    if (opts.blend_radii.has_value())
    {
        if (opts.blend_radii->size() == 0)
        {
            throw ValidationError("setOptions: blend_radii cannot be empty.");
        }
...
const Limits& TrajectoryGenerator::limits() const
{
    if (!limits_.has_value())
        throw ValidationError("Limits not set.");
    return *limits_;
}
